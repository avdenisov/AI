...
class CornersProblem(search.SearchProblem):
  ...
  def __init__(self, startingGameState):
    ...
    "*** YOUR CODE HERE ***"
    "https://github.com/weixsong/pacman/blob/master/searchAgents.py"
...
def cornersHeuristic(state, problem):
  ...
  "*** YOUR CODE HERE ***"
  "based on https://github.com/weixsong/pacman/blob/master/searchAgents.py"
  position = state[0]
  ...
  "my code - the full travelling salesman optimization"
  unvisited_corners = []
  corner_distances = []
  ...
  #print "unvisited_corners", unvisited_corners, len(unvisited_corners)
  
  def MinPath(start,unvisited):
      if unvisited == []:
          return 0
      dist = float('inf')
      for node in unvisited:
          unvisited1 = unvisited[:]
          unvisited1.remove(node)
          #print dist, start, node, util.manhattanDistance(start, node), unvisited, unvisited1, MinPath(node,unvisited1)
          dist = min(dist, util.manhattanDistance(start, node) + MinPath(node,unvisited1))
          #print dist
      return dist
 
  #print "MinPath", MinPath((1,1), [(0,0),(0,4),(4,0)])
  ...
  for i in range(0, 4):
      if corner_state[i] == 0:
          ...
          pos1 = position
          pos2 = corners[i]
          unvisited_corners1 = unvisited_corners[:]
          unvisited_corners1.remove(pos2)
          dist = util.manhattanDistance(pos1, pos2) + MinPath(pos2,unvisited_corners1)
          if cost == None:
              cost = dist
          else:
              if cost > dist:
...
def foodHeuristic(state, problem):
  ...
  def findClosestDist(current_pos, targets):
      idx = -1
      min_dist = None
      for i in range(len(targets)):
          dist = util.manhattanDistance(current_pos, targets[i])
...
class ClosestDotSearchAgent(SearchAgent):
  ...
  def findPathToClosestDot(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    foods = food.asList()
    #A Star search will find the closest food

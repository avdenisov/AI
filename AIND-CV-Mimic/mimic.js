...
var targetEmoji = emojis[Math.floor(Math.random() * emojis.length)];
...
function setScore(correct, total) {
  $("#score").html("Score: " + correct + " / " + total); // + " --- " + tarEm + " <> " + curEm);
}
...
detector.addEventListener("onImageResultsSuccess", function(faces, image, timestamp) {
  ...
  // my code begin
  // log('#results', "Facial Point 0: x = " + faces[0].featurePoints[0]["x"] + ", y = " + faces[0].featurePoints[1]["y"]);
  // my code end
  if (faces.length > 0) {
    ...
    log('#results', "Emoji: " + faces[0].emojis.dominantEmoji); // + " " + toUnicode(faces[0].emojis.dominantEmoji));
...
function drawEmoji(canvas, img, face) {
  ...
  // <your code here>
  ctx.font = '96px serif';
  ctx.fillText(face.emojis.dominantEmoji,  face.featurePoints[11]["x"] + 100, face.featurePoints[11]["y"] + 50); // at the level of nose tip
...
function playGame(canvas, img, face) {
  var currentEmoji = toUnicode(face.emojis.dominantEmoji);
  setScore(correctScore,totalScore, targetEmoji, currentEmoji);
  if (timeLastEmoji === 0 || timeLastEmoji + 30 < Math.floor(Date.now() / 1000)) {
    targetEmoji = emojis[Math.floor(Math.random() * emojis.length)];
    setTargetEmoji(targetEmoji);
    totalScore++;
    setScore(correctScore,totalScore);
    timeLastEmoji = Math.floor(Date.now() / 1000);
  };
  if (currentEmoji === targetEmoji) {
    ...
    // totalScore++;
    // setScore(correctScore,totalScore, targetEmoji, currentEmoji);
    timeLastEmoji = 0; // Math.floor(Date.now() / 1000);
  }
}
